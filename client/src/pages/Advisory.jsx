...
const Advisory = () => {
    ...
    const generateAdvisory = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.msg || err.response?.data?.errors?.[0]?.msg || t('Error generating advisory'));
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="page-content">
                    ...
                    <div className="flex gap-3 items-center">
                        {data && data.advisories.length > 0 && (
                            <button
                                ...
                                className={`font-semibold px-4 py-2 rounded-xl shadow-sm text-white text-sm transition-colors ${isSpeaking ? 'bg-red-500 hover:bg-red-600' : 'bg-[#065f46] hover:bg-[#059669]'}`}
                            >
                                {isSpeaking ? t('Stop') : t('Listen')}
                            </button>
                        )}
                        <Link to="/dashboard" className="text-[#059669] hover:text-[#065f46] font-semibold text-sm">&larr; {t('Dashboard')}</Link>
                    </div>
                </div>

                {/* Demo Overrides Panel */}
                <div className="glass-card rounded-2xl p-4 mb-6">
                    <h3 className="font-bold text-[#374151] mb-2">{t('Simulate Demo Conditions (Optional)')}</h3>
                    <div className="flex flex-col sm:flex-row gap-4 mb-4">
                        <div className="flex items-center">
                            ...
                            <label htmlFor="mockWeatherActive" className="text-sm font-medium text-[#374151]">{t('Use Mock Weather')}</label>
                        </div>
                        {mockWeatherActive && (
                            <div className="flex gap-2">
                                <input type="number" name="temperature" value={mockWeather.temperature} onChange={handleMockWeatherChange} placeholder={t('Temp(C)')} className="w-20 p-1 border rounded text-sm" />
                                <input type="number" name="humidity" value={mockWeather.humidity} onChange={handleMockWeatherChange} placeholder={t('Hum(%)')} className="w-20 p-1 border rounded text-sm" />
                                <input type="number" name="forecastRainfall" value={mockWeather.forecastRainfall} onChange={handleMockWeatherChange} placeholder={t('Rain(mm)')} className="w-24 p-1 border rounded text-sm" />
                            </div>
                        )}
                    </div>
                    <div className="flex flex-col sm:flex-row gap-4 items-center">
                        <input type="text" value={manualGrowthStage} onChange={(e) => setManualGrowthStage(e.target.value)} placeholder={t('Manual Stage (e.g. tillering)')} className="p-2 border rounded text-sm flex-1" />
                        <button onClick={generateAdvisory} disabled={loading} className="bg-green-600 hover:bg-[#065f46] text-white font-bold py-2 px-4 rounded disabled:opacity-50">
                            {loading ? t('Generating...') : t('Regenerate Advisory')}
                        </button>
                    </div>
                </div>
                ...
                {data && (
                    <>
                        {/* Top Section Summary */}
                        <div className="glass-card rounded-2xl p-6 border-t-4 border-indigo-500 mb-6">
                            <h2 className="text-xl font-bold mb-4">{t('Advisory Summary')}</h2>
                            <div className="flex flex-wrap gap-4">
                                <div className="bg-red-50 text-red-800 px-4 py-2 rounded-full font-bold border border-red-200">
                                    {data.summary.critical} {t('Critical Alerts')}
                                </div>
                                <div className="bg-orange-50 text-orange-800 px-4 py-2 rounded-full font-bold border border-orange-200">
                                    {data.summary.high} {t('High Priority Alerts')}
                                </div>
                            </div>

                            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm bg-gray-50 p-4 rounded-lg">
                                <div><strong>{t('Growth Stage')}:</strong> <span className="capitalize">{data.meta.growthStageUsed}</span></div>
                                {data.weatherUsed ? (
                                    <>
                                        <div><strong>{t('Temp')}:</strong> {data.weatherUsed.temperature}°C</div>
                                        <div><strong>{t('Humidity')}:</strong> {data.weatherUsed.humidity}%</div>
                                        <div><strong>{t('Rainfall')}:</strong> {data.weatherUsed.forecastRainfall}mm</div>
                                    </>
                                ) : (
                                    <div className="col-span-3 text-red-600 font-semibold whitespace-nowrap">{t('Weather Data Unavailable')}</div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <h3 className="text-xl font-bold text-gray-800 mb-2">{t('Detailed Advice')}</h3>
                            {data.advisories.length === 0 ? (
                                <p className="text-[#6b7280]">{t('No specific advisories generated for this combination of conditions.')}</p>
                            ) : (
                                data.advisories.map((adv) => (
                                    <div key={adv.id} className={`p-4 rounded-md border-l-4 shadow-sm ${getSeverityClasses(adv.severity)}`}>
                                        ...
                                        <p className="text-xs opacity-75 italic block">{t('Source')}: {adv.source}</p>
                                    </div>
                                ))
                            )}
                        </div>
