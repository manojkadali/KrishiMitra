        ...
        <div className="min-h-screen flex">
            ...
            <div className="hidden lg:flex lg:w-[55%] gradient-hero flex-col justify-between p-14 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <h2 className="text-4xl font-extrabold text-white leading-tight mb-5 tracking-tight animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                        {t('Join thousands of smart farmers.')}
                    </h2>
                ...
                <div className="relative z-10 bg-white/8 backdrop-blur-sm rounded-2xl p-6 border border-white/10 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
                    ...
                    <p className="text-emerald-300/60 text-xs mt-3 font-medium">— Farmer from Andhra Pradesh</p>
                </div>
            </div>
            ...
            <div className="w-full lg:w-[45%] flex items-center justify-center px-6 py-12 bg-[#f0f4f3]">
                <div className="w-full max-w-[380px] animate-fade-in-up">
                    ...
                    <h1 className="text-[28px] font-extrabold text-[#1a2e2a] mb-1.5 tracking-tight">{t('Create account')}</h1>
                    <p className="text-sm text-[#4b6360] mb-8">
                        {t('Already have an account?')}{' '}
                        <Link to="/login" className="text-[#059669] font-semibold hover:text-[#065f46] transition-colors">
                            {t('Sign in')}
                        </Link>
                    </p>
                    ...
                    <div className="relative mb-6">
                        ...
                        <div className="relative flex justify-center text-xs">
                            <span className="px-4 bg-[#f0f4f3] text-[#9ba8a3] font-medium">{t('or register with email')}</span>
                        </div>
                    </div>

                    <form onSubmit={onSubmit} className="space-y-5">
                        <div>
                            <label className="block text-xs font-semibold text-[#4b6360] mb-2 uppercase tracking-wider">{t('Full name')}</label>
                            <input name="name" type="text" required value={name} onChange={onChange} className="input-field" placeholder="Your full name" />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-[#4b6360] mb-2 uppercase tracking-wider">{t('Email')}</label>
                            <input name="email" type="email" required value={email} onChange={onChange} className="input-field" placeholder="you@example.com" />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-[#4b6360] mb-2 uppercase tracking-wider">{t('Password')}</label>
                            <input name="password" type="password" required value={password} onChange={onChange} className="input-field" placeholder="Minimum 6 characters" />
                        </div>
                    ...
                    <p className="text-[11px] text-[#9ba8a3] text-center mt-6">
                        {t('register_footer')}
                    </p>
                </div>
            </div>
